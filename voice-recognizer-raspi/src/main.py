...
# when you are debugging this you need to execute:
# systemctl stop voice-recognizer
# before you kill it from htop so it doesn't restart immediately
# don't forget to run it (python -i ./src/main.py) from the start dev terminal which sets up the env


import goatchurchcode   # contains the coffee making checklist commands
...
# useful for access when running as python -i
Dplayer = None
Dsay = None
Drecorder = None
Drecognizer = None
...
eq_filter = tts.create_eq_filter()  # improves sound quality
...
piblips = "12,20 "  # signifies the local pi speaking
...
def main():
    ...
    Dplayer = player
    action.VolumeControl(None, 100).run("max volume")   # start loud at the start (which is not very loud)
...
def do_recognition(args, recorder, recognizer, player):
    ...
    mic_recognizer = SyncMicRecognizer(actor, recognizer, recorder, player, say, triggerer, led_fifo=args.led_fifo)
    ...
    with mic_recognizer:
        ...
        while True:
            i += 1
            if mqttclient:
                mqttclient.loop(timeout=0.2)   # needs to be called regularly (otherwise would need its own thread)
            else:
                time.sleep(0.2)
            if (i%100) == 0:
                logger.info("tick")
...
audio.shortjblips(500, 0.8)

nonmqttcommands = ["volume up", "volume down", "max volume", "IP address"]
class SyncMicRecognizer(object):
    ...
    def recognize(self):
        ...
        self.recognizer_event.set()
        audio.shortjblips(400, 0.8)   # double beep to acknowledge recording
        audio.shortjblips(400, 0.8)

    def endpointer_cb(self):
        self.recorder.del_processor(self.recognizer)
        audio.shortjblips(400, 0.4)   # double beep to acknowledge finished recording
        audio.shortjblips(400, 0.4)
    ...
    def _handle_result(self, result):
        ...
        if result.transcript in ["repeat what you said", "what did you say"]:
            blipsay(piblips+"I said "+heardhistory[-1])
            return
        if result.transcript in ["repeat what you heard", "repeat back", "what did you hear", "what did I say"]:
            blipsay(piblips+("I heard %d words: "%len(spokenhistory[-1].split())) +spokenhistory[-1])
            return
        
        if mqttclient and result.transcript and result.transcript not in nonmqttcommands:
            spokenhistory.append(result.transcript)
            mqttclient.publish(mqttspoken, result.transcript)
            return

        if result.transcript and goatchurchcode.process(result.transcript, self.say):
            return
