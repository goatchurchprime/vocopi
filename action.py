...
VolumeControl(None, 100)   # say it loud at the start
...
class junk(object):
    ...
    def run(self, voice_command):
        # The command still has the 'repeat after me' keyword, so we need to
        # remove it before saying whatever is left.
        comm = voice_command.replace(self.keyword, '', 1).strip()
        print("comm", comm)
        if comm == "on":
            publish.single("ESPURNA_3B6F59/relay/0/set", "1", hostname="mqtt.local")
            self.say(_("It is on"))
        elif comm == "off":
            publish.single("ESPURNA_3B6F59/relay/0/set", "0", hostname="mqtt.local")
            self.say(_("It should be off"))
        else:
            self.say(("What?"))
...
def make_actor(say):
    ...
    actor.add_keyword(
        _('list directory'), SpeakShellCommandOutput(
            say, "ls",
            _('what.')))
...
def add_commands_just_for_cloud_speech_api(actor, say):
    ...
    simple_command('alexa', _("Alexa who?"))
    simple_command(
        'beatbox',
    ...
    simple_command(_('tell me a joke'),
                   _('Why did the toilet paper roll down the hill? To get to the bottom.'))
    simple_command(_('three laws of robotics'),
                   _("kill all humans"))
    simple_command(_('where are you from'), _("the dustbin."))
