...
def main():
    ...
    tts.say(player, "Booting up 1 2 3 4 5", eq_filter, "en-US")
    ...
    if mqttbroker:
        ...
        def on_connect(client, userdata, flags, rc):
            ...
            tts.say(player, "M Q T T Connected", eq_filter, "en-US")
        def on_disconnect(client, userdata, rc):
            print("on_disconnect", client, userdata, rc)
            tts.say(player, "M Q T T Disconnected", eq_filter, "en-US")
        ...
        def on_message(client, userdata, message):
            ...
            tts.say(player, message.payload, eq_filter, "en-US")
...
class SyncMicRecognizer(object):
    ...
    def _handle_result(self, result):
        ...
        if result.transcript in ["repeat what you said", "what did you say"]:
            self.say("I said "+heardhistory[-1])
            return
        if result.transcript in ["repeat what you heard", "repeat back", "what did you hear", "what did I say"]:
            self.say(("I heard %d words: "%len(spokenhistory[-1].split())) +spokenhistory[-1])
            return
